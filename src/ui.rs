...
use bevy_egui::{EguiContexts, egui::{Button, FontId, self, Context}};
...
impl Plugin for NovelUiPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<UiLayerStack>()
            ...
            .add_systems(PreUpdate, react_window_resize)
...
pub fn set_default_style(mut contexts: EguiContexts) {
    let ctx = contexts.ctx_mut();
...
pub fn react_window_resize(
    ...
    if let Some(window) = resize_reader.into_iter().last() {
        ...
        ctx.set_pixels_per_point(window_size / TARGET_SIZE);
    }
}